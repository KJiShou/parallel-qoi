...
std::vector<int> runSerialWildfire(const std::vector<int>& initialGrid, const SimulationConfig& cfg) {
...
int main() {
    ...
    const std::vector<int> finalGrid = runSerialWildfire(initialGrid, cfg);
    const auto end = std::chrono::high_resolution_clock::now();

    const double milliseconds = std::chrono::duration<double, std::milli>(end - start).count();
    printResult("Serial CPU", milliseconds, finalGrid);
