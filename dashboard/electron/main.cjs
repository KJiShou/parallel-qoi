...
function registerIpc(){ ipcMain.handle('wildfire:get-capabilities',()=>({electron:true,liveRuns:true,cudaBlockSizes:[128,256,512],backends:['serial','openmp','cuda','mpi']})); ipcMain.handle('wildfire:start-run',(_event,config)=>manager.start(config)); ipcMain.handle('wildfire:start-trace',(_event,config)=>manager.startTrace(config)); ipcMain.handle('wildfire:cancel-run',(_event,runId)=>manager.cancel(runId)); ipcMain.handle('wildfire:list-runs',()=>manager.list()); ipcMain.handle('wildfire:read-run',(_event,runId)=>manager.read(runId)); }
