...
function buildTraceCommand(config,{rootDir,buildDir=path.join(rootDir,'build_verified','Release'),traceSteps}={}){
  if(!Number.isInteger(traceSteps)||traceSteps<1||traceSteps>2000) throw new Error('traceSteps must be an integer from 1 to 2000');
  if(config.rows*config.cols*traceSteps>2_000_000_000) throw new Error('trace workload exceeds the trusted work budget');
