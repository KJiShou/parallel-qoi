...
function buildTraceCommand(config,{rootDir,buildDir=path.join(rootDir,'build_verified','Release')}={}){
  const tracePath=path.join(rootDir,'results','live',`trace-${config.rows}x${config.cols}-${Date.now()}.json`);
  return {file:path.join(buildDir,'wildfire_serial.exe'),args:[...baseArgs(config),'--repetitions','1','--warmup','0','--frames',tracePath,'--frame-interval','5','--trace-only'],tracePath};
}
