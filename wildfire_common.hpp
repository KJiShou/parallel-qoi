...
inline void writeCompressedFrame(std::ostream& out, const SimulationConfig& cfg, const std::vector<int>& grid, int step, bool& first) {
    const int aggregationRows = std::max(1, cfg.rows / 500), aggregationCols = std::max(1, cfg.cols / 500); std::vector<std::uint8_t> packed(62500, 0), burningMask(31250, 0);
    for (int row = 0; row < 500; ++row) for (int col = 0; col < 500; ++col) { int counts[4] = {0,0,0,0}; for (int r = row * aggregationRows; r < std::min(cfg.rows, (row + 1) * aggregationRows); ++r) for (int c = col * aggregationCols; c < std::min(cfg.cols, (col + 1) * aggregationCols); ++c) ++counts[grid[index2D(r,c,cfg.cols)]]; const int priority[] = {BURNING,BURNED,TREE,EMPTY}; int state = EMPTY, maximum = -1; for (int candidate : priority) if (counts[candidate] > maximum) { state = candidate; maximum = counts[candidate]; } const int index = row * 500 + col; packed[index / 4] |= static_cast<std::uint8_t>(state << ((index % 4) * 2)); if (counts[BURNING] > 0) burningMask[index / 8] |= static_cast<std::uint8_t>(1u << (index % 8)); }
    if (!first) out << ",\n"; first = false; out << "    {\"step\": " << step << ", \"cells2BitBase64\": "; writeJsonString(out, base64Encode(packed)); out << ", \"burningMaskBase64\": "; writeJsonString(out, base64Encode(burningMask)); out << "}";
}
